...
| `reason`      | Sanitized deny reason (from the model, or `GENERIC_DENY_REASON` fallback). Present only on deny verdicts; absent for allow/defer. The circuit-breaker gate uses a static `BREAKER_DENY_REASON` instead                                                                                                                                                                     |
