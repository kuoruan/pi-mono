...
import { buildReportCandidates, templateBashTarget } from "#src/report.ts";
...
describe("report module invariants (source scan)", () => {
  it("imports no write APIs and writes no files (never writes, never auto-applies)", () => {
    const source = readFileSync(new URL("../src/report.ts", import.meta.url), "utf8");
    const readerSource = readFileSync(
      new URL("../src/decision-log-reader.ts", import.meta.url),
      "utf8",
    );
    for (const [name, src] of [
      ["report.ts", source],
      ["decision-log-reader.ts", readerSource],
    ] as const) {
      // No fs imports at all — the reader's file access is injected.
      expect(src.includes('from "node:fs"'), `${name} must not import node:fs`).toBe(false);
      // No write-family calls in the module body.
      for (const banned of [
        "writeFile",
        "appendFile",
        "writeFileSync",
