...
**Circuit breaker**:
Per-session, two-tier, fail-safe. `consecutive` is a recoverable tier (trips after N consecutive denies, resets so the model gets another chance); `total` is a hard session cap (never resets on its own — only `/ai-guard breaker reset` or a restart clears it). Breaker trips are NOT counted as model denials. Deny-equivalents — machinery-failure denials (the modes that deny them) AND `strict`'s model-defer→deny mapping — count into `consecutive` only: a broken or persistently uncertain reviewer can trip the recoverable tier like a miscalibrated one, but an equivalent storm never burns the permanent `total` tier (that one stays model-denies-only). The total tier's first trip notifies the operator once at error grade (re-armed by a reset; the notice is the ambient channel's only error line — `notifyLevel` `off` silences it and consumes the once-per-epoch claim, while the `error` threshold keeps it visible); `/ai-guard breaker reset` clears both tiers — pure in-memory counter mutation (nothing persisted, nothing else touched; resume builds fresh breaker state regardless).
