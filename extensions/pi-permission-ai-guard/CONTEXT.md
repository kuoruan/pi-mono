...
**Operator notices**:
The notify-line doctrine (all lines go through `ui.notify`; the footer channel is mode-status only). Skeleton: event sentence + em-dash consequence + parenthesized qualifier; state echoes (`mode = permissive (session override)`) are the deliberate exception. A model deny that holds or escalates notifies in every mode (v27 renders no dialog for denials — the line is the operator's only copy); a mode-softened deny ends `— asking you instead`; a deny that holds needs no tail; `permissive` swallows soft denies whole (zero-interruption contract — the one-time fail-open notice covers it). Model reasons go out whole with a 240-char defensive ceiling (`NOTIFY_REASON_CEILING`); the audit record keeps the full text regardless. Machinery denies stay silent (fail-closed needs no announcement); machinery defers and deferring breaker trips always name their cause.
