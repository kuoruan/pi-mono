...
import { isRecord, sanitizeForPrompt, truncate } from "./utils.ts";
...
function toolCallsFromAssistant(content: unknown, maxChars: number): string[] {
  ...
  for (const block of content) {
    ...
    const sanitized = sanitizeForPrompt(argStr);
    calls.push(`${name}: ${truncate(sanitized, maxChars)}`);
  }
  return calls;
}
...
export function stripTranscript(
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (hasSummary(entry)) {
      // Compaction/branch summaries are pi-generated summaries of prior context.
      // They are trusted because pi generates them, NOT the agent. However,
      // if fromHook is true, the summary was generated by an extension (which
      // may incorporate model output), so we treat those as untrusted.
      if (entry.fromHook === true) {
        strippedCount++;
        continue;
      }
      const summary = entry.summary;
      if (summary && trustedIntent.length < options.maxUserMessages) {
        trustedIntent.push(truncate(sanitizeForPrompt(summary), options.maxCharsPerEntry));
      }
      continue;
    }
    ...
    if (role === "user") {
      ...
      if (text && trustedIntent.length < options.maxUserMessages) {
        trustedIntent.push(truncate(sanitizeForPrompt(text), options.maxCharsPerEntry));
      }
      continue;
    }
