# Open issue: grammar-state test flakes under full parallel runs (10-50%) — root-cause carrier removed by the engine migration

## Status update: the root-cause carrier left with the engine migration

The shiki engine migration (JS-regex → Oniguruma WASM) removed the flake's identified root-cause carrier from the runtime: `oniguruma-to-es`'s `lazyCompileLength` machinery (emulated long-pattern regexes compiled on first `exec`) no longer exists in the tokenize path — the WASM engine interprets TextMate patterns directly. Leading suspects 1-2 below are `oniguruma-to-es` internals (lazy long-pattern compilation, `EmulatedRegExp.clip_search`); suspect 3 concerns that dependency under vitest forks. The dependency left our runtime graph entirely, so none of them can recur. Post-migration evidence: 5 consecutive full-parallel runs green (562/562 each — the old signature was 10-20% per run). Not marked definitively closed: CI accrual continues, and the in-test self-heal (clear cache + re-render, ≤3 attempts) stays as unrelated defense-in-depth. The guardrails below remain binding regardless.
...
- **`fileParallelism: false` is stable** (4× green, ~26s vs ~7s) — the suite ran sequentially until this was rooted upstream. This pin was later REPLACED by an in-test self-heal (see the Status update above): the whole suite runs parallel again.
...
- `fileParallelism: false` was the last-resort stopgap; the config has been `true` since the self-heal landed (the bullet above).

## Leading suspects (historical record — the carrier dependency left the runtime; kept for the forensic trail)
...
- `corePromise` (the shiki core singleton) must NEVER gain a test reset: it is the one state where an engine-level corruption would persist — the process is its only isolation boundary. `resetPigmentForTest` in the fixtures deliberately stops short of it.
- The highlight-cache reset seam (`clearHighlightCacheForTest`) exists for the in-test self-heal and stays available for any future mitigation; the re-tokenize recovery question itself is moot (Next step 2).
