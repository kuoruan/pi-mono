 ...
 * accept `{ prefix, stamps, identity }` and the precomputed string would silently win, dropping
 * every stamp the call site just listed (the exact drift this type exists to prevent).
 */
export type PreviewIdentity =
  | { prefix: string; stamps: Array<string | number>; identity?: never }
  | { identity: string; prefix?: never; stamps?: never };
...
export function definePreviewTask(spec: DefinePreviewTaskSpec): PreviewTask {
  const identity =
    spec.identity !== undefined ? spec.identity : taskKeyOf(spec.prefix, spec.stamps);
  return {
    identity,
