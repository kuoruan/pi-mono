...
describe("definePreviewTask (the task builder)", () => {
  ...
  it("rejects a literal carrying BOTH identity shapes (the stamps would be dropped silently)", () => {
    // The union declares each member's other keys as `never`, so the
    // directive below IS the assertion: without the never-negatives the
    // excess-property rule accepts this literal, and the runtime
    // consequence is asserted too — the precomputed string wins and the
    // stamp list is discarded, which is exactly the drift the type now
    // forbids at compile time.
    // @ts-expect-error -- the derived and precomputed shapes are exclusive
    const task = definePreviewTask({
      prefix: "wd",
      stamps: ["never", "used"],
      identity: "PRECOMPUTED-WINS",
      widthAware: true,
      placeholder: "P",
      fallback: "F",
      invalidate: () => {},
    ...
    expect(task.identity).toBe("PRECOMPUTED-WINS");
  });
