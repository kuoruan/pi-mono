...
- Header ownership is tool-kind based, and the shell badge now rides the CALL header: all non-shell tools render body-only (the call header already names the tool, so a validation-failure frame stays a bare message); shell tools' failures carry the parsed status badge as the command echo's `·`-separated suffix ("✗ exit N" — 128-255 the signal range, "✗ timeout Ns", "✗ aborted", "✗ terminated"), so a RECOGNIZED status line renders body-only here too, and the frame's own bare name header remains only for an unrecognized shell failure (degraded, never broken).
