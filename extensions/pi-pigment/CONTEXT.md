...
**Error frame**: The errored-result rendering (`formatToolErrorResult`), invoked by the factory's renderResult error branch for EVERY wrapper: drafts the message at width, paints the box background from the theme's `toolErrorBg`, and pre-wraps each visual row so every continuation carries the indicator-bar column. The shell badge taxonomy (exit/signal/timeout/abort/terminate kinds) lives beside it, not in the call-header module.
