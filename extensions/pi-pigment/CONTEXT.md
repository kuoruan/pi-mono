...
- the call slot renders argument feedback (label, path, command, streaming counts) AND the result summary once it lands — bridged through render state: edit's `1 edit (12 diff lines) +2 -1`, write's `✓ new file (N lines)` / `+N -M` / `✓ no changes`, all in the same header-suffix position; the shell tools keep the header a bare prompt line (`$ cmd` — the same header-suffix family grammar) and, once the call settles, append a status suffix after a muted `·` separator in that same suffix position — symmetric at both ends, success `· ✓`, failure `· ✗ exit 1` (error-frame's worded badge; onError bridges the message's status line into state)
...
**Error frame**: The errored-result rendering (`formatToolErrorResult`), invoked by the factory's renderResult error branch for EVERY wrapper: drafts the message at width, paints the box background from the theme's `toolErrorBg`, and pre-wraps each visual row so every continuation carries the indicator-bar column. The shell badge taxonomy (exit/signal/timeout/abort/terminate kinds) lives beside it, not in the call-header module. Its measured `Took` footer takes the bar's kind color; an unmeasured row renders no footer.

- Header ownership is tool-kind based, and the shell badge rides the CALL header: all non-shell tools render body-only (the call header already names the tool, so a validation-failure frame stays a bare message); shell tools' failures carry the parsed status badge as the command echo's `·`-separated suffix ("✗ exit N" — 128-255 the signal range, "✗ timeout Ns", "✗ aborted", "✗ terminated"; a no-match exit 1 from grep/test/diff-family commands dims to muted), so a RECOGNIZED status line renders body-only here too, and the frame's own bare name header remains only for an unrecognized shell failure (degraded, never broken).
...
- One tail grammar (`... (N more lines, <keyHint> to expand)` — the key hint resolves the user's binding and composes byte-identically to pi's own `keyHint`, "ctrl+o" is only the default): the collapsed window advertises the expand key; an expanded cap reports the remainder without an affordance (nothing further to expand into); the `Took Xs` footer reads the render-state execution clock and paints success green — a tail footer exists only on a settled, successful call; the SDK's limit notice (below) rides its own line under the tail.
- The SDK's limit notice is derived from `result.details` — the same structured flags pi's native renderers read (`matchLimitReached`/`resultLimitReached`/`entryLimitReached`/`linesTruncated`/`truncation`) — and lifted out of the body the output memo hands the wrappers: it never renders as a grep hit, a find path, or an ls tree row (it used to wear a `├──` connector), never spends the collapse budget, and always shows. The text shape alone is never the authority: a bracketed filename stays a path.
...
**Command highlight**: The shell tools' (bash/powershell) call header — the command rendered in shell grammar over a toolTitle base (uncolored tokens inherit it), swapped in async once args complete. The status suffix (a muted `·`, then the settled frame's mark — success "✓", failure "✗ exit N" — the same suffix-state grammar as write's "✓ new file") composes around it fresh at every setText — never inside the highlight cache (the cache stays command-only). The command's language is known by definition (it IS shell), so no guessing; the output renders through the SDK's native result renderer (delegation, lastComponent withheld). _Avoid_: output-language guessing from the command (misfires are worse than plain).
